...
실험 1 비교군 전용이다. 운용 `go2_rl_spawn.launch.py` 를 **그대로 복사해**
컨트롤러만 바꿨다. 의도적으로 베낀 것이다 — 스폰 순서·이벤트 체인·관절
시딩을 직접 다시 짠 하네스로는 로봇이 기립조차 못 했다. 컨트롤러 성능을
재는 실험이므로 스폰 쪽 조건은 운용과 한 글자도 달라선 안 된다.
**운용 런치를 고치면 이 파일도 같이 봐야 한다.**
