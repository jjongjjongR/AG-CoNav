...
모듈 A의 드론 2.5D 지도와 모듈 D의 wheel·leg 2.5D 지도를 공통 `map` 좌표와 `0.10 m/cell` 격자 규약에 맞춰 검증·배치하고, 합집합 범위·중복 셀 처리 규칙(드론은 폴백, wheel·leg는 `elevation_variance` 비교 — 아래 참고)·`NaN` 보존 규칙을 적용하여 **wheel·leg 두 로봇의 지도 생성 완료 상태가 모두 수신된 시점에 1회** 통합 2.5D 지도로 병합하며, 완성된 지도를 제공하고 저장한다.
...
8. 중복 셀 처리: 드론은 wheel·leg 둘 다 미관측인 셀에만 쓰이는 최하위 폴백이고, wheel·leg가 겹치는 셀은 각 지도의 `elevation_variance`(모듈 D 칼만필터 불확실성) 레이어를 비교해 분산이 더 작은(더 확신 있는) 쪽의 값을 채택 (동률이면 wheel — 6-4 참고)
...
3. 출력 격자 + 세 지도의 셀별 값 → 중복 셀 처리 적용 : 드론은 wheel·leg 모두 미관측인 셀의 폴백, wheel·leg 겹침은 `elevation_variance`가 더 작은 쪽 채택(동률 시 wheel), 유효값은 `NaN`으로 덮지 않음
...
- 중복 셀 처리: 드론은 wheel·leg 둘 다 미관측인 셀에만 쓰이는 최하위 폴백. wheel·leg가 겹치는 셀은 `elevation_variance`(모듈 D 칼만필터 불확실성)가 더 작은 쪽 채택, 동률이면 wheel (6-4 참고, 구 `wheel > leg > 드론` 고정 우선순위에서 변경)
...
- **하는 일**
  ...
  - 각 지도의 셀 값을 출력 격자에 배치. 드론은 wheel·leg 모두 미관측인 셀에만 쓰이는 최하위 폴백으로 먼저 배치하고, 그 위에 wheel·leg가 겹치는 셀은 `elevation_variance`가 더 작은(더 확신 있는) 쪽 값을 채택해 덮어씀 (동률이면 wheel, 6-4 참고 — 입력 순회 순서에 의존하지 않는 결정적 처리, 벡터화 연산)
...
| 필수 레이어 | `elevation` |
| 부가 레이어 | `elevation_variance` — wheel·leg(모듈 D)는 항상 포함, 드론(모듈 A)은 이번 작업 범위 밖이라 없을 수 있음. 없으면 `extract_elevation_variance`가 `None`을 반환하도록 방어적으로 처리(6-4/8-5 참고) |
...
| 병합 규칙 | 드론은 wheel·leg 둘 다 미관측인 셀에만 쓰이는 최하위 폴백. wheel·leg가 겹치는 셀은 `elevation_variance`가 더 작은 쪽 채택(동률이면 wheel). 유효값은 `NaN`으로 덮지 않음, 셋 다 미관측인 셀만 `NaN` (구 `wheel > leg > 드론` 고정 우선순위에서 변경 — 변경 이력 참고) |
...
| 필수 레이어 | `elevation` |
| 부가 레이어 | `elevation_variance` (wheel·leg만 필수로 취급, 드론은 없어도 됨 — 8-1 참고) |
...
| 해상도 | `0.10 m/cell` |
| 지도 범위 | 세 입력 지도의 합집합 |
| 중복 셀 처리 | 드론은 wheel·leg 모두 미관측인 셀의 폴백, wheel·leg 겹침은 `elevation_variance`가 더 작은 쪽 채택(동률 시 wheel) |
...
| 중복 셀 | 드론은 wheel·leg 둘 다 미관측인 셀에만 쓰이는 폴백이고, wheel·leg가 겹치는 셀은 `elevation_variance`가 더 작은 쪽을 채택(동률이면 wheel)해야 하며, 유효값을 `NaN`으로 덮지 않아야 함 |
...
- **중복 셀 처리 확인:** 드론은 wheel·leg 둘 다 미관측인 셀에만 폴백으로 쓰이고, wheel·leg가 겹치는 셀은 `elevation_variance`가 더 작은 쪽이 채택되며(동률이면 wheel), 입력 순서와 무관하게 결과가 항상 동일하고, 유효값이 미관측 값으로 덮어써지지 않으며, 이상치가 지도 전체를 왜곡하지 않는다.
...
- 중복 셀 처리는 파이썬 딕셔너리나 set 등 순서가 보장되지 않는 자료구조에 의존하지 말고, 결정적(deterministic) 순서로 처리할 것 (드론 배열 먼저 덮어쓰기 → 그 위에 wheel·leg를 `elevation_variance` 비교로 벡터화 병합한 값만 유효한 곳에 덮어쓰기 — 동률은 wheel로 고정. 6-4/변경 이력 참고).
...
> **셀 선택 규칙 변경: `wheel > leg > 드론` 고정 우선순위 → 드론 폴백 + wheel/leg 분산 비교 (인터페이스 변경)**: 모듈 D(`agconav_ground_mapping`)의 `ground_elevation_mapper`가 셀별 칼만필터를 도입하면서 `elevation_map`에 `elevation_variance`(셀별 불확실성) 레이어를 새로 발행하게 됐고, 이 레이어가 모듈 E의 병합 우선순위를 더 정확하게 대체할 수 있게 됐다. 이에 따라 `elevation_map_merger`의 중복 셀 처리를 바꿨다: 드론(모듈 A)은 여전히 최하위 우선순위 폴백으로 유지(wheel도 leg도 그 셀을 관측하지 못했을 때만 사용) — 드론은 이번 작업 범위가 아니라 `elevation_variance`가 없을 수 있어, 드론 값을 분산 비교에 끌어들이지 않는다. wheel과 leg가 겹치는 셀만 `elevation_variance`가 더 작은(더 확신 있는) 쪽의 `elevation` 값을 채택하도록 바꿨다 — 두 분산이 완전히 같은 극단적인 경우는 결정성을 지키기 위해 wheel을 타이브레이커로 고정했다(구 `MERGE_ORDER`가 wheel을 최우선으로 뒀던 것과 일관).
>
> `grid_math.py`에 `extract_elevation_variance` 함수를 추가했다 — `elevation_variance` 레이어가 없는 지도(현재는 드론)에서 호출해도 `KeyError` 없이 `None`을 반환하도록 방어적으로 구현했다. 같은 파일에 `place_in_output_grid` 헬퍼를 추가해, 로봇별 지도를 출력 격자 크기로 정렬시키는 로직을 `map_merge_collector`의 격자 정렬 검증과 `elevation_map_merger`의 셀 선택 양쪽에서 재사용할 수 있게 했다. `MERGE_ORDER`는 이제 `('drone',)`만 남아 드론의 폴백 배치 순서만 나타낸다 — wheel/leg 사이의 선택은 더 이상 고정 순서가 아니라 분산 비교이기 때문이다.
>
> 전체 로직은 `output_grid` 크기의 배열 전체에 대해 벡터화로 구현했다(파이썬 셀 루프 없음). design.md 6-4/9-5의 "유효값은 `NaN`으로 덮지 않음" 원칙은 그대로 유지된다. 이 변경으로 `elevation_map`(wheel/leg 발행분)의 실제 레이어 구성이 하나(`elevation`)에서 둘(`elevation`, `elevation_variance`)로 바뀐 것은 모듈 D 쪽 인터페이스 변경이며, 그에 대응해 모듈 E가 이를 소비하도록 바뀐 것이 이번 변경이다 — 관련 내용은 `agconav_ground_mapping/design.md`의 변경 이력도 함께 참고.
