...
def generate_launch_description():
    ...
    declare_walking_algo = DeclareLaunchArgument(
        "walking_algo", default_value="champ", description="Walking algorithm (champ or nmpc)"
    )
    ...
    quadruped_controller_node = Node(
        ...
        condition=launch.conditions.LaunchConfigurationEquals("walking_algo", "champ")
    )
    
    nmpc_controller_node = launch.actions.ExecuteProcess(
        cmd=["python3", os.path.join(get_package_share_directory("agconav_bringup"), "..", "..", "..", "src", "agconav_bringup", "scripts", "nmpc_controller_node.py")],
        output="screen",
        condition=launch.conditions.LaunchConfigurationEquals("walking_algo", "nmpc")
    )
