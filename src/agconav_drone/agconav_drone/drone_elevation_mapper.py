...
7. 칼만필터 적용: ground_elevation_mapper(Module D)가 이미 적용한 것과 동일한
   ...
   (self._elevation_a/self._variance_a, 아래 9번 항목 참고)로 교체했다. 측정
   노이즈(R)는 거리(구간표 조회) + 입사각(np.gradient로 이전 elevation에서
   근사) + 점밀도를 결합해 계산하며(`_measurement_noise` 참고), 프로세스
   노이즈 Q는 정적 지형 가정 하에 0으로 고정, 이미 값이 있는 셀에는 이노베이션
   게이팅을 적용한다(`_kalman_update_cells` 참고). 거리 구간표
   (measurement_noise_max_distances/measurement_noise_sigmas)는
   ...
   `elevation_variance` 레이어를 elevation과 함께 새로 발행하며(9번 항목의
   혼합모델에서 선택된 가설의 값), agconav_map_fusion은 이제 wheel/leg
   겹침 셀에서 이 분산을 직접 비교하는 방식으로 바뀌었다(README 132행의
   구 고정 우선순위 서술은 낡은 내용) - 다만 드론 자신은 그 비교에 참여하지
   않고 wheel/leg 둘 다 못 본 셀의 최하위 폴백으로만 쓰인다
   (agconav_map_fusion/grid_math.py 참고).

8. 이상치 방어 2겹(Module D의 방어 로직과 동일한 설계): (1) `_accumulate`에서
   센서 원점 기준 거리가 `max_sensor_range`를 넘는 점을 격자에 넣기 전에
   버린다(np.isfinite로는 못 거르는, 유한하지만 물리적으로 말이 안 되게 먼
   점 대비 - gz gpu_lidar는 최대 사거리 밖 반사를 Inf가 아니라 유한한 큰
   값으로 채우는 경우가 있을 수 있어 1차 방어와 별개로 필요). (2)
...
9. 건물경계 혼합모델(mixture model, 1-8까지의 기본 칼만필터 위에 얹는 추가
   레이어): 셀별로 두 개의 독립 칼만필터 가설을 병렬로 추적한다 - 가설 A
   ("지형", self._elevation_a/_variance_a)와 가설 B("지붕/장애물 경계" 후보,
   self._elevation_b/_variance_b). 표면 법선(np.gradient) 추정은 계속 가설
   A(지형)만 참조한다 - 가설 B는 아직 확정 안 된 이상치 후보라 지형 표면
   추정을 오염시키면 안 된다.
   - 신규 셀(가설 A조차 없음): 가설 A를 이번 배치로 초기화(게이팅 없음).
     가설 B는 아직 없음(NaN) - 기존과 동일한 콜드스타트.
   - 가설 A만 있는 셀: 새 관측의 이노베이션이 게이트(9.0)를 통과하면 A를
     그대로 갱신(1-5의 표준 칼만 갱신). **통과 못하면(기본 칼만필터라면
     버렸을 관측) 그 관측으로 가설 B를 새로 만든다** - 이게 핵심 차이,
     관측을 버리지 않고 "두 번째 후보"로 보존한다.
   - A/B가 둘 다 있는 셀: 새 관측을 정규화 이노베이션(innovation**2 /
     (P+R))이 더 작은(더 가까운) 가설에만 붙여 그 가설만 갱신한다. 둘 중
     더 가까운 쪽조차 게이트를 통과 못하면(둘 다 통과 못한 것과 동치 -
     "더 가까운" 쪽의 정규화 이노베이션이 항상 더 작으므로) 3번째 후보를
     만들지 않고 이번 배치를 그냥 버려 복잡도를 2가설로 제한한다.
   - 최종 발행(`_build_grid_map_message`): 셀마다 분산(P)이 더 작은(더
     신뢰도 높은) 가설을 채택해 elevation/elevation_variance로 내보낸다.
     B가 아직 없는(NaN) 셀은 당연히 A가 채택된다. 동률이면 A(지형)를
     우선한다.
   `_grow_to_fit`도 네 배열(A의 elevation/variance, B의 elevation/variance)을
   항상 같은 shape/origin으로 함께 NaN 패딩하도록 확장했다.

   사전 검증: 이전 세션에서 이 정확한 설계(2가설 병렬 추적)를 "변형 11"로
   Module A의 실제 비행 bag(5m AGL)에 대해 A/B 비교 실험한 결과가 있다 -
   wheel FN% -6.30pp(상대 59.0% 개선), leg FN% -3.61pp(상대 45.8% 개선),
   커버리지도 +0.09pp 개선, 최대연결덩어리%도 wheel 27.58%->51.63%,
   leg 40.96%->54.51%로 크게 개선되어(FN%만 좋아 보이는 함정이 아님을
   확인) 실험한 11개 변형(게이팅 완화/Huber 재가중/R 모델 개선 3종 등) 중
   전체 최고 성적으로 채택되었고, Phase 2 조합 실험에서도 다른 변형들과
   병행 적용 시 이득이 거의 없어(중복 효과) 이 혼합모델 단독 채택이
   최종 결론이었다. 다만 그 실험 세션 자체는 코드를 실험 종료 후 baseline
   으로 완전히 되돌렸다(문서만 남기고 코드는 미반영) - 이번이 그 결론을
   실제 코드에 반영하는 첫 커밋이다. 게이팅 완화(16.0)나 Huber 재가중과는
   원래 실험에서도 상호배타적으로 다뤄졌으므로(같은 "게이트 실패 시 뭘
   할지" 지점을 다른 방식으로 바꾸는 변형들) 이번 구현에도 섞지 않았다 -
   게이팅 임계값은 9.0(3-시그마)을 그대로 유지한다.
"""
...
class DroneElevationMapper(Node):
    ...
    def __init__(self):
        ...
        # Tracking and Navigation"). Module D와 동일. 건물경계 혼합모델(9번
        # 항목)의 가설 A/B 게이팅에도 동일하게 쓰인다.
        ...
        # 건물경계 혼합모델(top-of-file 9번 항목): 가설 A("지형")가
        ...
        # 배열이) 0으로 패딩되면 "완벽하게 확신한다"는 뜻이 되어 그 셀의
        # 칼만 게인 K = P/(P+R)가 영구히 0으로 고정되는 버그가 된다
        # (_grow_to_fit 참고).
    ...
    def _accumulate(self, msg, transform):
        ...
        if row_idx is None:
            ...
            # _grow_to_fit이 이미 버렸다 (에러 로그도 거기서 남겼다). 기존
            # 누적(A/B 둘 다)은 그대로 보존된 상태이니 여기서도 조용히 이번
            # 배치만 버린다.
            return
    ...
    def _measurement_noise(
        ...
        Module D의 _measurement_noise와 동일한 설계. 표면 법선은 가설
        A("지형", top-of-file 9번 항목)에서만 추정한다.
        """
        ...
        # (self._elevation_a이 아직 NaN)이거나, 이웃들이 np.gradient에 충분한
    ...
    def _kalman_update_cells(self, rows, cols, batch_mean, r_eff):
        ...
        건물경계 혼합모델(top-of-file 9번 항목) -- 셀별로 최대 2개의 독립
        스칼라 칼만필터 가설(A="지형", B="지붕/장애물 경계" 후보)을 병렬로
        추적한다. Q=0(정적 지형 가정), 관측모델이 이미 선형(z = x + noise)
        이라 표준 (선형) 칼만필터로 충분하고 EKF/UKF 근사가 필요 없다는 점은
        1-5의 기본 칼만필터와 동일.
        """
    ...
    def _grow_to_fit(self, row_idx, col_idx):
        ...
        observed, it is never pre-sized. 건물경계 혼합모델(top-of-file 9번
        항목): 가설 A/B의 네 배열(elevation/variance x2)을 항상 같은
        shape/origin으로 함께 패딩한다.

        2겹 방어(top-of-file 8번 항목, 1겹은 _accumulate의 거리 필터): 패딩/
        생성 직전에 예상 총 셀 개수가 max_grid_cells를 넘으면 실제 배열
        확장/생성을 하지 않고 (None, None)을 반환한다 -- 기존 누적은 손대지
    ...
    def _select_hypothesis(self):
        ...
        건물경계 혼합모델(top-of-file 9번 항목)의 최종 발행 규칙: 분산(P)이
