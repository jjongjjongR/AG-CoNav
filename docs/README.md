...
무엇을 왜 그렇게 정했는지의 근거가 전부 여기 있다. 실행되는 코드는 없다.

원래는 `src/agconav_test_worlds` 패키지 안에 월드·런치·설정과 뒤섞여 있었다.
확정된 것은 전부 운용 패키지로 옮기고, 그 패키지는 지웠다. 실험용 자산을
운용 코드 옆에 남겨 두면 "실제로 쓰는 것"과 "실험용"이 섞여 어느 쪽이
진짜인지 알 수 없게 된다.
...
| 13 | `13. 컨트롤러 재실험 — 등판·단차·속도.md` |
| 14 | `14. 한계점 — unitree_guide 를 운용에 못 쓴 이유.md` |
