...
## [새 세션] gicp-preprocessing 재개(2회차) — A(다운샘플) 마저 완주, 표/결론 갱신

**사용자 응답 불가 상태로 재개, 전체 타임아웃 3시간.** 직전 세션이
UTM 크래시로 중단됐다는 전제로 재개.

### -1. 재개 지점 파악 결과
- `pgrep -af "ros2|gz sim|docker"` → dockerd 외 없음, gicp-gt-pose 관련
  프로세스 없음(안전 경계 위반 없음).
- 현재 브랜치: `gicp-preprocessing`(이미 체크아웃돼 있음, origin과 동기화).
- `git log` HEAD = `f289ae4`("조합(B+C) 완료 -- 개별보다 우수, GT-only
  격차 크게 축소") — 즉 **0~6단계(A 생략, B/C/조합, 결론, 문서화·커밋)가
  이미 완료돼 정상 push까지 끝난 상태**였음.

### 손상/불완전 여부 확인 — 크래시는 완료된 작업이 아니라 "추가 시도" 도중 발생
- `git status`: `run_results/logs/preprocessing_experiment.log`만
  unstaged(커밋된 내용 이후 이어붙은 로그).
- 로그를 시각 순으로 대조한 결과, `f289ae4` 커밋(20:32) **이후** 별도
  세션이 A(다운샘플, 이전엔 시간 예산으로 생략됐던 항목)를 채우려
  시도한 흔적을 발견: `BUILD start: voxel0.10`(01:41:34) →
  `BUILD done`(01:53:08) → `SCORE start/done`(01:53:08~02:09:00,
  wheel 43.74%/leg 32.96%) → 곧바로 `BUILD start: voxel0.20`(02:09:00)
  까지는 로그가 남아있었으나, voxel0.20 빌드 도중(스캔 27,627개 중
  약 27,400, ~99% 지점) 로그가 뚝 끊김.
- `uptime`으로 VM이 최근(당시 기준 약 3분 전) 재부팅됐음을 확인,
  `/tmp/gicp_preproc/`가 통째로 사라진 것도 재확인 — **이전에 이미
  확인된 동일 패턴("UTM 크래시 → VM 재부팅 → `/tmp` 휘발")이 그대로
  재현**된 것으로 판단. `f289ae4`까지의 코드/bag/B·C·조합 결과는
  전부 git에 커밋돼 있어 무사, 유실된 건 voxel0.10(완주했었지만
  `/tmp`에만 있던 결과)·voxel0.20(미완주) 산출물뿐.
- bag(`velocity_4m_5mps_preproc_src`, 27.1GiB) `ros2 bag info`로 정상
  인식 확인 — 손상 없음.

### 디스크 확인
`df -h ~` → 78%(17G 여유), 별도 정리 없이 진행 가능하다고 판단(이전
세션과 동일한 판단 근거 — bag 자체는 지울 수 없는 유일한 후보).

### 판단: A(0.10m/0.20m)를 마저 완주하기로 자율 결정
과제 원문이 A×2를 요구하고 있고, 이번 세션 예산이 넉넉하며(이전 세션이
생략한 이유는 "그 세션의" 남은 시간 부족이었지, A 자체가 무의미하다는
근거는 아니었음), 직전 세션도 실제로 A를 채우려 시도했던 정황(크래시
직전 로그)이 있어 **A를 생략된 채로 남겨두지 않고 완주하는 쪽이 과제
취지에 더 부합**한다고 판단. `setsid`+`nohup`+`disown`으로 세션과
완전히 분리해 `run_preprocessing_experiment.sh ... voxel0.10 voxel0.20`
재실행(voxel0.10을 처음부터 다시 돌린 건, 그 결과도 `/tmp`에만 있어서
크래시로 함께 유실됐기 때문 — 재실행 자체는 낭비가 아니라 필요한 조치).

### 실행 결과
- **중간 체크포인트**: 재개 확인 직후 로그만 먼저 커밋/push(`df9ee94`)
  — 백그라운드 실행이 다시 끊기는 최악의 경우에도 재개 지점 기록은
  보존되도록.
- voxel0.10 재실행: BUILD 02:31:48→02:43:26, SCORE →02:58:15,
  **wheel FN%=39.78%, leg FN%=31.62%, 커버리지 88.42%**(GICP 성공
  27,014/27,598). 크래시 전 1차 시도(43.74%/32.96%, GICP 성공 26,985)와
  비교하면 GICP 성공 스캔 수 차이 0.1% 수준의 런투런 변동 — 부동소수점/
  스레드 순서 영향으로 판단, 재실행 결과를 최종 채택.
- voxel0.20: BUILD 02:58:16→03:05:49, SCORE →03:20:26, **wheel
  FN%=51.39%, leg FN%=43.06%, 커버리지 86.95%**(GICP 성공 27,146/27,598).
  0.10m 대비 개선 효과가 거의 없음(전처리없음 대비 -1.55pp/-1.35pp뿐).
- 결과 JSON을 즉시 `/tmp`에서 `run_results/methodb_voxel0.10_fn.json`
  `/methodb_voxel0.20_fn.json`으로 영속 복사(재휘발 방지).
- `gicp_preprocessing_result.md` 0/1/2/3/5/6/7절 갱신: A 결과 반영,
  효과 순위 확정(**C > A0.10 ≈ B > A0.20**, A0.20은 사실상 무의미),
  A0.10을 B+C 조합에 추가하는 3중 조합은 이번 세션에서 실행하지 않고
  6절에 열린 질문으로 명시.

### 정리/저장
잔여 프로세스 없음 확인(`pgrep`). 디스크 상태 이상 없음. 이 세션에서
바뀐 파일(`gicp_preprocessing_result.md`, 이 절, `preprocessing_experiment.log`,
`methodb_voxel0.10_fn.json`/`methodb_voxel0.20_fn.json`) 전부 커밋/push
대상 — 이 커밋으로 완료.
...
### [이어서] 사용자 복귀 후 combo 재실행 — 2차 중단 후 완주

사용자가 "집왔어 이어서 진행하자"로 복귀. 재개 확인(브랜치/프로세스/
디스크 전부 이상 없음, `/tmp/gicp_preproc` 깨끗) 후 combo를 다시 시작.

**1차 재시도 — 예상 밖의 원인으로 다시 중단(단, 이번엔 VM 재부팅
아님).** `run_in_background`로 시작한 빌드가 스캔 15,400개(약 56%)
처리 시점에 "killed" 상태로 끊김. 조사 결과 `uptime -s`가 여전히
17:57(직전 크래시 시점)을 가리켜 **VM 재부팅은 없었다** — `dmesg`/
`journalctl`에서도 OOM-kill이나 세그폴트 등 크래시 시그니처를 찾지
못했다. `journalctl` 로그에 18:24경 gnome-shell이 재초기화되는 패턴
(최초 부팅 때와 유사한 xkbcomp 에러 반복)이 보여, 랩탑 절전/재연결
같은 이벤트가 이 세션이 추적하던 백그라운드 프로세스 트리를 끊은 것으로
추정(확정은 아님). 이번엔 `/tmp`가 휘발되지 않아 부분 산출물
(`baseline_combo.npy.raw` 2.3GB, `methodb_combo.npy.raw` 2.1GB)이
남아있었으나, 중간 지점에서 재개할 방법이 없어(스트리밍 방식이라
바이트 오프셋 신뢰 불가) 삭제하고 처음부터 재시작.

**2차 재시도 — `setsid`+`nohup`+`disown`으로 세션과 완전히 분리해
재실행, 완주.** 같은 종류의 중단이 반복될 가능성에 대비해, 이번엔
백그라운드 작업 자체를 이 세션(및 그 프로세스 그룹)과 완전히 분리해
실행 — 세션이 다시 끊겨도 실제 작업은 OS 레벨에서 계속되도록 함. 진행
모니터링도 로그 파일 전체가 아니라 **이번 시도 시작 시점 이후의 줄만**
(오프셋 기준) 보도록 수정(첫 모니터 구현은 로그가 append-only라는 걸
간과해 이전 시도들의 "SCORE start" 잔여 줄 때문에 "프로세스 죽음" 감지
로직이 무력화되는 버그가 있었음 — 두 번째 모니터에서 PID 기반 생존
확인 + 줄 오프셋으로 수정). 스캔 15,400 지점(1차 재시도가 끊겼던
지점)을 무사히 통과, 27,598 스캔 전부 처리 후 **20:14:19 BUILD
done(exit 0)**, 채점까지 **20:28:32 ALL DONE**으로 정상 완주.

**결과**: wheel FN% 21.44%, leg FN% 16.04%, 커버리지 89.26% — B/C
개별 결과보다 훨씬 좋음(자세한 수치·해석은
`gicp_preprocessing_result.md` 4·5절). 결과 JSON을 `/tmp`가 다시
휘발되는 것에 대비해 `run_results/methodb_combo_fn.json`으로 영속
경로에 복사해두고, `/tmp/gicp_preproc`는 정리(baseline_combo.npy 4GB
포함, 재생성 가능한 스크래치라 삭제 — 디스크 84%→78% 회복).

### 저장/보고 (combo 완료 후)
`gicp_preprocessing_result.md`(2·3·4·5·6·7절 갱신/신규), `run_results/
methodb_combo_fn.json`(신규), 이 PROGRESS.md 절, `preprocessing_experiment.log`
전부 커밋 대상. push까지 완료 예정.
