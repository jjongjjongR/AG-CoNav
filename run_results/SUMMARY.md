# SUMMARY — FAST-LIO2 + GPS 사후결합 후속 실험 (2026-08-20) — 채택 비권고

**목표: 순수 FAST-LIO2(GPS 없음) 실험이 남긴 저커버리지(30.63%) 한계에
GPS 사후결합을 적용하면 개선되는지 확인.** FAST-LIO2 재실행 없이
이미 저장된 궤적에 GLIM 실험용으로 만든 범용 `glim_gps_correct.py`를
그대로 재사용(재비행/재실행 없음, 몇 분 내 완료).

## 결과 — GLIM 사후결합 실험과 정확히 같은 실패 패턴 재현
| 지표 | GPS 없음 | GPS 사후결합 | 차이 |
|---|---|---|---|
| 커버리지 | 30.63% | **98.75%** | +68.12pp |
| wheel FN% | 2.97%⚠️ | **97.50%** | +94.53pp(대폭 악화) |
| 최대연결덩어리%(wheel/leg) | 13.54%/15.09% | **0.00%/0.00%** | 완전 붕괴 |

고도값 실측(버그 아님): std=2.46m, 범위 -30.0~43.3m — 5m AGL 비행에서
불가능한 국소 노이즈가 커버리지 확장과 함께 그대로 드러남.

## 핵심 결론 — 가설이 빗나갔다, 정직하게 기록
GPS 결합을 제안할 때 "FAST-LIO2의 드리프트는 GLIM처럼 진동형이 아니라
거의 선형/체계적이니 국소 정합은 신뢰할 만할 것"이라는 가설을 세웠으나
**틀렸다.** GPS로 이전엔 박스 밖이라 채점에서 빠졌던 후반 38% 비행
구간을 강제로 끌어오자, 그 구간 자체의 국소 정합이 이미 심하게
나빠져 있었다는 게 드러났다 — 전역 위치가 선형으로 표류했다고
국소 정확도가 보장되는 게 아니었다. **"GPS 사후결합은 저주파만 고치고
국소 노이즈는 못 고친다"는 GLIM의 결론이 도구(GLIM/FAST-LIO2)나
드리프트 모양(진동형/선형형)에 상관없이 사후결합 방식 자체의 구조적
한계임을 재확인.** FAST-LIO2 GPS 사후결합도 채택 비권고로 결론.

상세 근거: `run_results/fastlio_gps_postfusion_result.md`, 원래 실험
결과의 6절도 이 결과를 반영해 갱신됨(`run_results/fastlio2_result.md`).

---

# SUMMARY — FAST-LIO2(spark-fast-lio) 실험 (2026-08-20)

**목표: FAST-LIO2(GLIM과 무관한 독립 ROS2 패키지, 완전히 별도
워크스페이스 `~/fastlio_ws`에 설치, 이 저장소 소스 무수정)를 5m
AGL/4m/5mps 조건에서 GPS 없이 단순평균 지도로 테스트, 순수 GLIM
실험(wheel FN 33.39%)과 사과 대 사과로 비교.**

## 0~1단계 — 빌드 성공, 버그 2건 발견·수정 후 발산 확인
`MIT-SPARK/spark-fast-lio` 1차 시도로 빌드 성공(ROS2 Jazzy 명시 지원).
model.sdf 실측으로 extrinsic 직접 계산(실측 데이터로 IMU프레임 Z가
5m AGL에 부합함을 검증). per-point timestamp가 GLIM과 달리 필수라
방위각 근사 브리지 신규 작성.

버그 2건: (1) 이종 dtype 읽기 크래시, (2) 무반사 Inf 점 미필터로
파국적 발산(수천 미터) — 둘 다 수정. 수정 후에도 거의 선형(등속)
드리프트 재현(GT 대비 t=10.28s부터 발산, 88초간 최대오차 132.85m).
시작시점·gravity_alignment on/off 대조까지 했으나 동일 패턴 —
GLIM에서도 확인된 이 센서 데이터 자체의 근본적 어려움(희소·편중
반사 패턴)으로 판단, 우회 시도 중단.

## 2~4단계 — 전체 bag 완주, 4가지 지표
전체 bag(2129.67초, 자유낙하 제외) **크래시 없이 완주**. 지도(단순평균)
구축 결과:

| 지표 | 값 |
|---|---|
| 비행 소요시간 | 2129.674초(35.49분) |
| 커버리지 | **30.63%**(부분 커버리지, 비행 초반에 편중) |
| wheel FN% / leg FN% | **2.97% / 2.28%**⚠️(낮은 커버리지 탓에 액면가 비교 불가) |
| wheel/leg 최대연결덩어리% | **13.54% / 15.09%** |

## 종합 비교
| 조건 | wheel FN% | leg FN% | 최대덩어리%(wheel/leg) | 커버리지 |
|---|---|---|---|---|
| baseline #5 | 10.91% | 8.03% | 27.58%/40.96% | 95.80% |
| 11(혼합모델) | 4.38% | 4.28% | 51.63%/54.51% | 95.98% |
| 순수GLIM | 33.39% | 33.12% | 0.00%/0.00% | 40.99% |
| **FAST-LIO2** | 2.97%⚠️ | 2.28%⚠️ | **13.54%/15.09%** | 30.63% |

**핵심 결론**: FAST-LIO2는 순수GLIM보다 최대연결덩어리%가 뚜렷이
낫고(0%→13~15%) 드리프트 성격도 더 체계적(거의 선형)이지만, 커버리지
(30.63%)가 baseline #5(95.80%)에 크게 못 미쳐 실무 대안은 못 된다.
FN%가 낮아 보이는 건 비행 초반의 편중된 조각만 채점됐기 때문 —
액면가로 "GLIM보다 정확하다"고 읽으면 안 됨(본문 4-3절 참고).
**GPS 결합은 GLIM 때보다 시도해볼 가치가 있다고 판단**(선형/체계적
드리프트는 주기적 GPS 재정렬이 잘 듣는 유형) — 단 커버리지 자체의
근본적 한계(애초에 그 위치를 못 봤다는 문제)는 GPS로도 못 고친다는
점을 감안해 제한된 목표로 접근할 것을 권고.

상세 근거는 `run_results/PROGRESS.md`("[새 세션] FAST-LIO2 실험" 절),
전체 보고서는 `run_results/fastlio2_result.md` 참고.

---

# SUMMARY — Module A 칼만필터 11가지 개선 후보 실험 + Phase 2 조합 (2026-08-20)

**목표: baseline #5(칼만필터+GT pose, wheel FN% 10.91%) 코드에서 정확히
하나만 바꾼 11가지 변형을 각각 독립 검증하고, 개선을 보인 것들을
조합해 최종 후보를 찾는다.** rate=2.0(baseline #5 대비 오차 <0.3pp로
검증)으로 재생 시간을 절반으로 줄여 11개+조합+최종검증을 8시간 예산
안에 전부 완료했다.

## Phase 1 결과 (11/11 전부 시도, 스킵 없음)

| 변형 | wheel FN% | Δ | 판정 |
|---|---|---|---|
| H(중앙값) | 12.55% | +1.87pp | 기각 |
| K(최소분산하한) | 11.02% | +0.34pp | 기각 |
| F(편향진단) | - | - | 치우침 없음, 기각 |
| 게이트=4.0 | 22.50% | +11.82pp | 기각(큰폭 악화) |
| 게이트=16.0 | 5.95% | -4.73pp | 채택 |
| G(Huber 재가중) | 4.66% | -6.02pp | 채택 |
| Q(process noise) | 10.73% | +0.05pp | 기각 |
| 7(점단위 사전게이팅) | 10.52% | -0.16pp | 기각 |
| 8(R 가중합) | 7.58% | -3.10pp | 채택 |
| 9(방향성 R) | 9.78% | -0.90pp | 채택(약함) |
| 10(적응형 R) | 5.82% | -4.86pp | 채택 |
| **11(건물경계 혼합모델)** | **4.38%** | **-6.30pp** | **채택(전체 1위)** |

(local baseline: wheel FN% 10.68%, leg FN% 7.89%, 커버리지 95.89% —
baseline #5 원 보고치와 오차 <0.3pp)

## Phase 2 — 조합해도 11 단독을 못 넘는다

게이트16/G/11은 "이노베이션이 임계값을 넘으면 무엇을 할지"를 다루는
같은 코드 지점의 상호배타적 대안이라, 최고 성적인 11을 게이팅 전략으로
채택하고 8+9+10(R모델 개선 3종, 서로 다른 코드 위치라 조합 가능) +
게이트16(11 내부 임계값)을 전부 얹어 테스트. **결과: wheel FN% 4.35%,
leg FN% 4.29% — 11 단독(4.38%/4.28%)과 사실상 통계적으로 구분 불가**
(커버리지는 오히려 0.09pp 낮음). R모델 개선 3종이 풀려던 근본 원인
(고정 R의 과신 → 좋은 관측도 하드게이트에 거부됨)을 11이 더 근본적인
층위(거부된 관측을 안 버리고 두 번째 가설로 보존)에서 이미 해결해버려,
추가 이득이 없는 "중복" 관계였던 것으로 해석.

## 최종 검증(지시된 최대연결덩어리% 코드로) — FN%뿐 아니라 실사용성도 확인
...
11/조합 둘 다 연결된 통과가능 영역 자체가 baseline보다 훨씬 넓다 —
"FN%만 좋아 보이고 실제로는 못 쓰는 지도"가 아님을 확인.

## 최종 추천
**11(건물경계 혼합모델) 단독 채택을 권고** — 8/9/10/게이트16을 추가로
얹을 이유가 없다(중복, 구현 복잡도만 증가). 구현이 부담되면 차선책은
G(Huber, 단일 블록 교체로 구현 훨씬 단순, FN%가 11과 0.24~0.34pp
차이). **`drone_elevation_mapper.py`는 모든 실험 후 baseline #5 상태로
완전히 복원**(git diff 0 확인 완료) — 어느 조합을 실제 코드에 반영할지는
사용자가 이 문서를 보고 직접 결정할 사항이며 이번 세션에서 코드에는
반영하지 않았다.

상세 근거는 `run_results/PROGRESS.md`("[새 세션] Module A 칼만필터
11가지 개선 후보 실험" 절), 종합 결과는 `run_results/kf_experiment_summary.md`,
개별 실험은 `run_results/kf_variant_*.md` 참고.

---

# SUMMARY — GPS 결합 2종(사후/실시간) 비교 + GLIM 실험 트랙 최종 결론 (2026-08-19)

**목표: 직전 세션의 순수 GLIM 궤적(wheel FN% 33.39%)에 A(사후결합)와
B(실시간결합, glim_ext gnss_global) 두 GPS 방식을 각각 적용해 비교하고,
GLIM 관련 실험 전체를 마무리 짓는 최종 결론을 낸다.**

## 결과
| 조건 | wheel FN% | leg FN% | 커버리지 |
|---|---|---|---|
| baseline #5(칼만필터+GT) | 10.91% | 8.03% | 95.80% |
| 순수GLIM(GPS없음) | 33.39% | 33.12% | 40.99% |
| A: GPS사후결합 | 99.76% | 98.00% | **99.98%**(커버리지만 개선, FN% 대폭 악화) |
| B: GPS실시간결합 | 8.69%⚠️ | 8.57%⚠️ | **6.41%**⚠️(표본이 너무 작아 비교 불가) |

**A(사후결합)**: translation만 보정, 회전/국소노이즈는 그대로라 커버리지는
크게 개선되지만(99.98%) 그동안 "미측정"으로 빠졌던 노이즈 낀 스캔들이
전부 "관측됨"으로 드러나며 FN%가 순수GLIM보다도 악화(99.76%) — 저주파
드리프트만 고치고 고주파 노이즈는 못 고친다는 게 핵심 발견.

**B(실시간결합)**: `libgnss_global.so` 정상 로드·활성화 확인(T_world_utm
로그), 자유낙하 구간도 이번엔 세그폴트 없이 정상 회피(모니터링 SIGINT
방식). 그러나 **라이브 파이프라인(bag play+glim_rosnode 3-프로세스
동시구동)에서 심각한 프레임 손실**(궤적 포즈가 배치방식의 45%만 남음)로
궤적이 사실상 못 쓰게 됨(커버리지 6.41%) — wheel FN% 8.69%가 낮아
보이지만 미측정 91%짜리 극소 표본이라 다른 결과와 비교 불가. 예전
Phase 4의 두 실패(세그폴트/GPS미활성화)와도 다른 **제3의 실패 양상**.

## GLIM 실험 트랙 최종 결론
GICP, 완전독립추정, 파라미터튜닝, 실시간GPS×2회, 사후결합GPS, 순수GLIM
표준재현까지 전부 시도한 결과: **GLIM 자체가 이 환경(GPU 없는 VM + 이
시뮬레이터 센서 특성)에서 신뢰할 수 있는 궤적을 못 만든다는 게 근본
원인이고, GPS를 사후/실시간 어느 쪽으로 붙여도 해결되지 않았다.**
baseline #5(GT pose 신뢰 + 셀단위 칼만필터+이상치방어)가 유일하게
안정적으로 좋은 결과를 내는 방법임이 최종 확정됐다 — 커스텀
파이프라인은 결과 포장이 아니라 실제로 필요했던 해법이었다.

상세 근거는 `run_results/PROGRESS.md`("[새 세션] GPS 결합 2종 비교" 절),
전체 보고서는 `run_results/pure_glim_gps_both_result.md` 참고.

---

# SUMMARY — 순수 GLIM 표준 재현 실험 (2026-08-19) — 커스텀 코드가 아니라 GLIM/환경 문제로 확정

**목표: 수동 청크분할·GPS 사후결합·메모리 가드 수정 등 모든 커스텀 우회를
걷어내고, GLIM 공식 문서가 권장하는 표준 방식 그대로 100x100 지역 2.5D
elevation map을 얻어 4가지 지표(비행시간/커버리지/wheel FN%/최대연결덩어리%)
를 산출. 허용 예외 2개(자유낙하 구간 제외, 격자크기 크래시방지 상한)만
적용, 정확도 개선용 파라미터 튜닝·GPS결합 없음.**

## 표준 설정 재검증
패키지(`ros-jazzy-glim` 1.2.2) 미수정 원본과 직접 diff 대조: 진짜 기본값은
GPU 조합(이 VM엔 GPU 없어 불가). 기존 실험이 쓰던 CT+passthrough+pose_graph는
teleport bag 대응용 특수 조합이었음을 재확인. 패키지에 이미 있던 진짜 CPU
표준(LiDAR-IMU tight coupling)과, 완전 미수정 원본값(constant_velocity_
inf_scale=1e3)의 CT를 각각 새로 구성해 대조 → CPU-IMU는 즉시 발산(151.5m),
CT가 상대적으로 나음(79.6m) → CT를 대표 표준으로 채택.

## 결과: GLIM 표준 파이프라인 자체가 이 환경에서 부정확 — 커스텀 코드 문제 아님
짧은 깨끗한 구간(1단계)부터 이미 GT 대비 수십~80m 벗어났고, 전체 bag(자유낙하
제외, t≤2129.67s)으로 확장해도 크래시/OOM 없이 완주했지만 같은 bounded-발산
양상(최대오차 199.94m)이 유지됐다. 커스텀 다운스트림 코드는 전혀 관여하지
않았으므로(원시 GLIM 궤적 → 단순평균 지도만 사용) **이 부정확성은 GLIM
자체의 문제로 확정.**
...
| wheel FN% | **33.39%**(leg FN% 33.12%) |
| wheel/leg 최대연결덩어리% | **0.00% / 0.00%** |

baseline #5(칼만필터+GT pose, wheel FN% 10.91%)와 비교하면 wheel FN%가
3배 이상 나쁘고, 통과가능 영역의 최대연결덩어리는 사실상 0%다. "GLIM+GPS
cv1e3+사후결합" 비교는 과거 세션에서 GLIM 궤적 발산으로 실행까지 못
가고 미완료로 남아있음을 git log/PROGRESS.md 전체 검색으로 확인, 숫자를
지어내지 않고 "미완료"로 표시.

**핵심 결론**: 이전 세션들의 커스텀 우회(GT pose 신뢰, 칼만필터
이상치방어, GPS 사후결합)는 결과를 인위적으로 좋게 포장한 게 아니라
GLIM 표준 파이프라인이 이 환경(GPU 없는 VM + 이 시뮬레이터 센서 특성)
에서 스스로 해결 못하는 실제 정확도 문제를 메우기 위해 필요했다.

상세 근거는 `run_results/PROGRESS.md`("[새 세션] 순수 GLIM 표준 재현
실험" 절), 전체 보고서는 `run_results/pure_glim_result.md` 참고.
