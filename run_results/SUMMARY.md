# SUMMARY — GLIM Phase 4 시도 (2026-08-17, glim_ext gnss_global 실시간 GPS 제약) — 미채택

**목표: 사후보정이 아니라 GLIM 최적화 안에 GPS를 실시간 제약으로 넣는
`glim_ext`의 `libgnss_global.so`를 cv1e3(ATE 68.6m)에 적용해 개선되는지
확인. 결과: 검증 실패로 미채택.**

- 1차 시도(전체 12청크): GPS 제약은 34초 만에 정상 활성화됐으나
  (`T_world_utm=` 로그 확인), 임무완료 후 자유낙하 구간(t≈2135s, 직전
  세션이 이미 식별한 구간)에서 GLIM이 빈 포인트클라우드를 받고
  **세그폴트**로 죽어 결과를 완전히 잃음. gnss_global의 추가 메모리
  사용이 이 크래시를 악화시킨 것으로 추정(같은 지점 RSS가 비-GNSS
  버전보다 높았음).
- 2차 시도(크래시 구간 제외, 11청크): 크래시 없이 완주했으나 **GPS
  제약이 이번엔 한 번도 활성화되지 않음**(GPS 데이터는 정상 수신 확인,
  그런데도 정렬 로그 전무) — 동일 설정인데 실행마다 결과가 다른
  신뢰성 문제 확인. ATE 81.8m(GPS 제약 없이 cv1e3가 11/12청크만 처리된
  것과 사실상 동일 — GPS 효과를 보여주는 숫자가 아님).
- **결론: glim_ext gnss_global은 "half-baked"라는 자체 경고대로
  실행마다 활성화가 일관되지 않아 이번 세션 범위에서 신뢰성 검증에
  실패했다.** 원인을 더 파는 대신, 이미 설계·구현이 끝나 있는 사후결합
  (`glim_gps_correct.py`)으로 다음 세션이 넘어갈 것을 권고 — 이 방식은
  주기적 재정렬 구조라 gnss_global의 "초기 1회 정렬 고정" 약점과
  실행별 비결정성 문제 둘 다 구조적으로 피할 수 있다.

상세 근거는 `run_results/PROGRESS.md`("Phase 4 — glim_ext gnss_global"
절) 참고.
