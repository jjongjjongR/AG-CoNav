#!/usr/bin/env bash
# Phase 2 검증용: 지정한 config로 glim_rosnode를 라이브로 띄운 채 청크
# 0..N-1을 순서대로(t필드 추가 -> 재생 -> 삭제) 먹인다. run_glim_chunked.sh를
# config_path/청크개수를 파라미터화한 버전.
# 사용: run_glim_chunked_param.sh <config_dir> <n_chunks> <out_name>
set -e
